...
/**
 * A pull stream is a wrapper around a node stream that allows us to pull instead
 * of the normal push semantics.
 */
class PullStream {
   ...
   *
   * @returns a string or null if the stream is done
   */
...
class LazyChunks {
  ...
  constructor(private pullStream: PullStream, config?: { maxChunks: number }) {
...
export default class NodeStreamSource implements ISource<string, string> {
  ...
  constructor(private src: Stream.Readable, config?: { maxChunks: number }) {
    this.lazyChunks = new LazyChunks(new PullStream(src), config);
  }
